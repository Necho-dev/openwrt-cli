...
::   Standalone     → pip install git+https://github.com/Necho-dev/openwrt-cli.git
...
if exist "%~dp0pyproject.toml" (
  ...
  %PYTHON% -m pip install %PIP_GIT% -q
)
if errorlevel 1 (
...
echo    %C_ACCENT%openwrt network interfaces%C_RESET%    %C_MUTED%# WAN / LAN / lo%C_RESET%
echo    %C_ACCENT%openwrt network neighbors%C_RESET%     %C_MUTED%# DHCP + vendor%C_RESET%
echo.
